...
class CategoryException extends Exception
{
    public static function createFailed(string $message, ?\Throwable $previous = null): self
    {
        ...
        return new self("Failed to create category: System error occurred.", 500, $previous);
    }
    ...
    public static function updateFailed(string $id, string $message, ?\Throwable $previous = null): self
    {
        ...
        return new self("Failed to update category: System error occurred.", 500, $previous);
    }
    ...
    public static function deletionFailed(string $id, string $message, ?\Throwable $previous = null): self
    {
        if ($previous && str_contains($previous->getMessage(), 'Constraint violation')) {
            return new self("Failed to delete: This category is being used by other data.", 409, $previous);
        }

        Log::error("Category Deletion Failed (ID: $id): $message", ['exception' => $previous]);
        return new self("Failed to delete category: System error occurred.", 500, $previous);
    }

    public static function inUse(string $message = 'Category is currently in use and cannot be deleted.'): self
    {
