    ...
    public function resolveKitToLoanItems(Kit $kit, ?int $preferredLocationId = null): array
    {
        ...
        foreach ($kit->items as $item) {
            ...
            if ($product->type === ProductType::Asset) {
                $assets = $this->findAvailableAssets($item->product_id, $neededQty, $targetLocationId);
                ...
                if ($assets->isEmpty()) {
                    $resolvedItems[] = [
                        ...
                        'product_name' => $product->name,
                        'item_id' => null,
                        'item_label' => "{$product->name} (Unavailable at Target)",
                        'quantity' => $neededQty,
                        'notes' => $item->notes,
                        'is_fallback' => true,
                    ];
                ...
                $stock = $this->findAvailableStock($item->product_id, $neededQty, $targetLocationId);
    ...
    public function getKitAvailability(Kit $kit, ?int $preferredLocationId = null): array
    {
        ...
        foreach ($kit->items as $item) {
            ...
            $details[] = [
                ...
                'is_enough' => $isEnough,
                'status' => $isEnough ? 'Available' : 'Insufficient',
            ];
        }
