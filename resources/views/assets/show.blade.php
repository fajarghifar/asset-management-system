            ...
            <div class="bg-card text-card-foreground shadow-sm sm:rounded-lg border border-border p-6">
                <div class="flex flex-col md:flex-row gap-8">
                    ...
                    <div class="w-full md:w-1/3 space-y-4">
                        @if($asset->image_path)
                            <img src="{{ Storage::url($asset->image_path) }}" alt="{{ __('Asset Image') }}" class="w-full h-auto rounded-lg border border-border object-cover shadow-sm">
                        @else
                            <div class="w-full h-64 bg-muted rounded-lg flex items-center justify-center text-muted-foreground border border-border border-dashed">
                                ...
                                <span class="ml-2">{{ __('No Image') }}</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="w-full md:w-2/3">
                        <div class="border-b border-border pb-4 mb-4">
                            <h4 class="text-base font-semibold text-foreground">{{ __('Asset Information') }}</h4>
                        </div>

                        <dl class="divide-y divide-border">
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 py-3">
                                <dt class="text-sm font-medium text-muted-foreground">{{ __('Product Name') }}</dt>
                                <dd class="text-sm text-foreground sm:col-span-2">{{ $asset->product->name }}</dd>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 py-3">
                                <dt class="text-sm font-medium text-muted-foreground">{{ __('Product Code') }}</dt>
                                <dd class="text-sm text-foreground sm:col-span-2">{{ $asset->product->code }}</dd>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 py-3">
                                <dt class="text-sm font-medium text-muted-foreground">{{ __('Asset Tag') }}</dt>
                                <dd class="text-sm text-foreground sm:col-span-2">{{ $asset->asset_tag }}</dd>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 py-3">
                                <dt class="text-sm font-medium text-muted-foreground">{{ __('Serial Number') }}</dt>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 py-3">
                                <dt class="text-sm font-medium text-muted-foreground">{{ __('Location') }}</dt>
                                <dd class="text-sm text-foreground sm:col-span-2">{{ $asset->location->full_name }}</dd>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 py-3">
                                <dt class="text-sm font-medium text-muted-foreground">{{ __('Purchase Date') }}</dt>
