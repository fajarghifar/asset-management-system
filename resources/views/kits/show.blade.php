        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-card text-card-foreground shadow-sm rounded-lg p-6 border border-border">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                    <div class="space-y-4">
                        <div>
                            <h3 class="text-lg font-medium border-b border-border pb-2 mb-4">{{ __('Kit Overview') }}</h3>
                            <dl class="space-y-4">
                                <div>
                                    <dt class="text-sm font-medium text-muted-foreground">{{ __('Kit Name') }}</dt>
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <dt class="text-sm font-medium text-muted-foreground">{{ __('Total Items') }}</dt>
                                        <dd class="mt-1 text-sm text-foreground">{{ $kit->items->count() }} {{ __('Products') }}</dd>
                                    </div>
                                </div>
                            </dl>
                        </div>
                    ...
                    <div>
                        <h3 class="text-lg font-medium border-b border-border pb-2 mb-4">{{ __('Description') }}</h3>
                ...
                <div class="space-y-6">
                    <div class="flex justify-between items-center">
                        <h3 class="text-lg font-medium text-foreground">{{ __('Availability Status') }}</h3>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border {{ $availability['is_fully_available'] ? 'bg-green-100 text-green-800 border-green-200' : 'bg-red-100 text-red-800 border-red-200' }}">
                            {{ $availability['is_fully_available'] ? __('Ready to Loan') : __('Incomplete') }}
                        </span>
                    </div>
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-2 text-sm text-yellow-700">
                                    <p>{{ __('Some items in this kit are not fully available in their specified locations.') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="overflow-visible border rounded-md">
                        <table class="w-full text-sm text-left">
                            <thead class="bg-muted text-muted-foreground uppercase text-xs">
                                <tr>
                                    <th class="px-4 py-3">{{ __('Product / Asset') }}</th>
                                    <th class="px-4 py-3">{{ __('Target Location') }}</th>
                                    <th class="px-4 py-3">{{ __('Qty Needed') }}</th>
                                    <th class="px-4 py-3">{{ __('Availability') }}</th>
